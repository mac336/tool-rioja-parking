...
import { Field, SelectField, Textarea, Button, Alert } from '@/components/ui'
...
export function RequestAccessPage() {
  ...
  const enviar = async () => {
    setEnviando(true)
    await crearSolicitud({ nombre, vivienda, email, comentario })
    ...
    <div className="min-h-dvh bg-bg">
      ...
      <Page className="mx-auto max-w-[560px]">
        ...
        <div className="mt-4 flex flex-col gap-4">
          <Field label="Nombre y apellidos" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej. Marta Ruiz Bel" />
          ...
          <Textarea label="Comentario (opcional)" value={comentario} onChange={(e) => setComentario(e.target.value)} placeholder="Cuéntanos brevemente quién eres…" />
