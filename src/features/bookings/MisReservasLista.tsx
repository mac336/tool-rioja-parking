...
const PILL: Record<ReservaEstado | 'celebrada', { label: string; cls: string }> = {
  ...
  celebrada: { label: 'Celebrada', cls: 'bg-info-soft text-info-ink' },
}
...
/** Lista de las reservas del usuario (sin cabecera). Reutilizada en la pantalla
 *  principal de Reservas y en "Mis reservas". */
export function MisReservasLista() {
  ...
  if (reservas.length === 0) {
    return <EmptyState titulo="Sin reservas" texto="Aún no has reservado ninguna zona común. Pulsa “Nueva reserva” para empezar." />
  }
