...
function PostItHome({ m, rot, onClick }: { m: Mensaje; rot: string; onClick: () => void }) {
  ...
  return (
    <div role="button" tabIndex={0} onClick={onClick}
      ...
      style={{
        background: e.paper, transform: `rotate(${rot})`,
      ...
      <div className="flex shrink-0 items-center justify-between gap-2">
        <TipoIcono tipo={m.tipo} color={e.tint} />
        <span className="px-2.5" style={{
          fontFamily: 'var(--font-hand)', fontSize: '16px', fontWeight: 600, color: e.tint, opacity: 0.85,
          transform: 'rotate(-2deg)', border: `1.5px solid ${e.tint}`, borderRadius: '55% 45% 50% 60% / 65% 55% 60% 50%',
        }}>{fechaMano(m.created_at)}</span>
      </div>

      <div className="mt-1 shrink-0 font-display text-[17px] font-extrabold leading-[1.22]"
      ...
      <div ref={ref} className="mt-1 min-h-0 flex-1 overflow-hidden">
            ...
            return (
              <div key={msg.id} className="flex h-full w-full shrink-0 items-center justify-center px-7 py-3">
                <div className="relative flex max-h-full w-full max-w-[330px] flex-col rounded-[10px] px-6 pb-6 pt-6"
                  style={{
                    background: pe.paper, transform: `rotate(${i % 2 ? '0.8deg' : '-0.8deg'})`,
                  ...
                  <div className="flex shrink-0 items-center justify-between gap-2">
                    <TipoIcono tipo={msg.tipo} color={pe.tint} size={20} />
                    <span className="px-3" style={{
                      fontFamily: 'var(--font-hand)', fontSize: '19px', fontWeight: 600, color: pe.tint, opacity: 0.85,
                      transform: 'rotate(-2deg)', border: `1.5px solid ${pe.tint}`, borderRadius: '55% 45% 50% 60% / 65% 55% 60% 50%',
      ...
      <div className="flex shrink-0 items-center justify-center gap-2 pb-1 pt-1" onClick={(ev) => ev.stopPropagation()}>
        {lista.map((msg, i) => (
          <button key={msg.id} onClick={() => setIdx(i)} aria-label={`Ver ${i + 1}`}
            ...
            style={{ width: i === idx ? 22 : 8, background: i === idx ? POSTIT[msg.tipo].pin : 'rgba(255,255,255,.28)' }} />
        ))}
      </div>
