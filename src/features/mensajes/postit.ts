...
export type MotivoKey = 'flor' | 'sol' | 'hoja' | 'calabaza' | 'copo' | 'mascara' | 'vela' | 'corazon' | 'warning' | 'problem' | 'foco'
...
export const TEMPORADAS: Record<EstiloTemporada, Temporada> = {
  ...
  idea: { etiqueta: 'Idea', paper: '#F3EAFB', tint: '#7A4FC0', pin: '#9B6FE0', pinHi: '#D7BEF5', deco: '#F5B417', motivo: 'foco' },
}

/** Orden del selector del formulario. */
export const TEMPORADAS_ORDEN: EstiloTemporada[] = ['primavera', 'verano', 'otono', 'halloween', 'navidad', 'valentin', 'carnaval', 'ssanta', 'warning', 'problem', 'idea']

/** Aspecto FIJO por tipo (decisión del usuario, v1.54.0). Incidencia y sugerencia
 *  no se eligen: se pintan siempre igual y el asistente ni muestra los selectores.
 *  El anuncio (y el aviso) sí los eligen. Claves de TEMPORADAS y de PASTELES. */
export const ASPECTO_FIJO: Partial<Record<MensajeTipo, { estilo: EstiloTemporada; color: string }>> = {
  incidencia: { estilo: 'problem', color: 'rosa' },
  sugerencia: { estilo: 'idea', color: 'lila' },
}
/** ¿El tipo deja elegir estilo/color en el asistente? */
export const eligeAspecto = (t: MensajeTipo): boolean => !(t in ASPECTO_FIJO)
