...
export function estadisticasReservas(): Promise<EstadisticasReservas> {
  ...
  for (const r of db.reservas) { const k = claveGrupo(r); if (!grupos.has(k)) grupos.set(k, { vivienda: r.vivienda, inicio: r.inicio, estado: r.estado }) }
  ...
  const ranking = [...veces.entries()].map(([vivienda, n]) => ({ nombre: vivienda, veces: n })).sort((a, b) => b.veces - a.veces)
